# @diotoborg/rerum-occaecati [![NPM version](https://img.shields.io/npm/v/@diotoborg/rerum-occaecati.svg?style=flat)](https://www.npmjs.com/package/@diotoborg/rerum-occaecati) [![NPM monthly downloads](https://img.shields.io/npm/dm/@diotoborg/rerum-occaecati.svg?style=flat)](https://npmjs.org/package/@diotoborg/rerum-occaecati) [![NPM total downloads](https://img.shields.io/npm/dt/@diotoborg/rerum-occaecati.svg?style=flat)](https://npmjs.org/package/@diotoborg/rerum-occaecati)  [![Tests](https://github.com/diotoborg/rerum-occaecati/actions/workflows/test.yml/badge.svg)](https://github.com/diotoborg/rerum-occaecati/actions/workflows/test.yml)
...
- [Sponsors](#sponsors)
  ...
  * [Why use @diotoborg/rerum-occaecati?](#why-use-@diotoborg/rerum-occaecati)
    + [Matching features](#matching-features)
  * [Switching to @diotoborg/rerum-occaecati](#switching-to-@diotoborg/rerum-occaecati)
...
```sh
$ npm install --save @diotoborg/rerum-occaecati
```
...
```js
const @diotoborg/rerum-occaecati = require('@diotoborg/rerum-occaecati');
// @diotoborg/rerum-occaecati(list, patterns[, options]);
```

The [main export](#@diotoborg/rerum-occaecati) takes a list of strings and one or more glob patterns:

```js
console.log(@diotoborg/rerum-occaecati(['foo', 'bar', 'baz', 'qux'], ['f*', 'b*'])) //=> ['foo', 'bar', 'baz']
console.log(@diotoborg/rerum-occaecati(['foo', 'bar', 'baz', 'qux'], ['*', '!b*'])) //=> ['foo', 'qux']
```

Use [.isMatch()](#ismatch) to for boolean matching:

```js
console.log(@diotoborg/rerum-occaecati.isMatch('foo', 'f*')) //=> true
console.log(@diotoborg/rerum-occaecati.isMatch('foo', ['b*', 'f*'])) //=> true
```

[Switching](#switching-to-@diotoborg/rerum-occaecati) from minimatch and multimatch is easy!

<br>

## Why use @diotoborg/rerum-occaecati?

> @diotoborg/rerum-occaecati is a [replacement](#switching-to-@diotoborg/rerum-occaecati) for minimatch and multimatch
...
## Switching to @diotoborg/rerum-occaecati

_(There is one notable difference between @diotoborg/rerum-occaecati and minimatch in regards to how backslashes are handled. See [the notes about backslashes](#backslashes) for more information.)_

### From minimatch

Use [@diotoborg/rerum-occaecati.isMatch()](#ismatch) instead of `minimatch()`:

```js
console.log(@diotoborg/rerum-occaecati.isMatch('foo', 'b*')); //=> false
```

Use [@diotoborg/rerum-occaecati.match()](#match) instead of `minimatch.match()`:

```js
console.log(@diotoborg/rerum-occaecati.match(['foo', 'bar'], 'b*')); //=> 'bar'
```
...
```js
console.log(@diotoborg/rerum-occaecati(['foo', 'bar', 'baz'], ['f*', '*z'])); //=> ['foo', 'baz']
```
...
```js
const mm = require('@diotoborg/rerum-occaecati');
...
```js
var mm = require('@diotoborg/rerum-occaecati');
...
```js
const { braces } = require('@diotoborg/rerum-occaecati');
...
```js
@diotoborg/rerum-occaecati(['a/b.js', 'a/c.md'], '*.js');
//=> []

@diotoborg/rerum-occaecati(['a/b.js', 'a/c.md'], '*.js', { basename: true });
//=> ['a/b.js']
```
...
```js
const files = ['abc', 'ajz'];
console.log(@diotoborg/rerum-occaecati(files, '[a-c]*'));
//=> ['abc', 'ajz']

console.log(@diotoborg/rerum-occaecati(files, '[a-c]*', { bash: false }));
```
...
const regex = @diotoborg/rerum-occaecati.makeRe('foo/{01..25}/bar', {
  expandRange(a, b) {
...
```js
const isMatch = @diotoborg/rerum-occaecati.matcher('*', { ignore: 'f*' });
...
```js
console.log(@diotoborg/rerum-occaecati(['a/z', 'a/b', 'a/!(z)'], 'a/!(z)'));
//=> ['a/b', 'a/!(z)']

console.log(@diotoborg/rerum-occaecati(['a/z', 'a/b', 'a/!(z)'], 'a/!(z)', { noextglob: true }));
...
```js
@diotoborg/rerum-occaecati(['a/b', 'a/b/c', 'a/b/c/d'], 'a/**');
//=> ['a/b', 'a/b/c', 'a/b/c/d']

@diotoborg/rerum-occaecati(['a/b', 'a/b/c', 'a/b/c/d'], 'a/**', {noglobstar: true});
//=> ['a/b']
```
...
const isMatch = @diotoborg/rerum-occaecati.matcher('*', { onIgnore, ignore: 'f*' });
isMatch('foo');
isMatch('bar');
isMatch('baz');
```
...
const isMatch = @diotoborg/rerum-occaecati.matcher('**', { onMatch, posixSlashes: true });
isMatch('some\\path');
isMatch('some\\path');
isMatch('some\\path');
```
...
const isMatch = @diotoborg/rerum-occaecati('*', { onResult, ignore: 'f*' });
isMatch('foo');
isMatch('bar');
isMatch('baz');
```
...
```js
console.log(@diotoborg/rerum-occaecati.match(['a\\b\\c'], 'a/**'));
//=> ['a/b/c']

console.log(@diotoborg/rerum-occaecati.match(['a\\b\\c'], { posixSlashes: false }));
//=> ['a\\b\\c']
```
...
```js
console.log(@diotoborg/rerum-occaecati.match(['abc', 'a\\*c'], 'a\\*c'));
//=> ['a\\*c']

console.log(@diotoborg/rerum-occaecati.match(['abc', 'a\\*c'], 'a\\*c', { unescape: true }));
//=> ['a*c']
```
...
```js
console.log(@diotoborg/rerum-occaecati.isMatch('a1', '[[:alpha:][:digit:]]')) //=> true
console.log(@diotoborg/rerum-occaecati.isMatch('a1', '[[:alpha:][:alpha:]]')) //=> false
```
...
However, it's suprising how many edge cases and rabbit holes there are with glob matching, and since there is no real glob specification, and @diotoborg/rerum-occaecati is more accurate than both Bash and minimatch, there are cases where best-guesses were made for behavior. In a few cases where Bash had no answers, we used wildmatch (used by git) as a fallback.

### Backslashes

There is an important, notable difference between minimatch and @diotoborg/rerum-occaecati _in regards to how backslashes are handled_ in glob patterns.
...
We made this decision for @diotoborg/rerum-occaecati for a couple of reasons:
...
Note that when you pass something like `path.join('foo', '*')` to @diotoborg/rerum-occaecati, you are creating a filepath and expecting it to still work as a glob pattern. This causes problems on windows, since the `path.sep` is `\\`.

In other words, since `\\` is reserved as an escape character in globs, on windows `path.join('foo', '*')` would result in `foo\\*`, which tells @diotoborg/rerum-occaecati to match `*` as a literal character. This is the same behavior as bash.
...
```sh
# .makeRe star
  @diotoborg/rerum-occaecati x 2,232,802 ops/sec ±2.34% (89 runs sampled))
  minimatch x 781,018 ops/sec ±6.74% (92 runs sampled))

# .makeRe star; dot=true
  @diotoborg/rerum-occaecati x 1,863,453 ops/sec ±0.74% (93 runs sampled)
  minimatch x 723,105 ops/sec ±0.75% (93 runs sampled)

# .makeRe globstar
  @diotoborg/rerum-occaecati x 1,624,179 ops/sec ±2.22% (91 runs sampled)
  minimatch x 1,117,230 ops/sec ±2.78% (86 runs sampled))

# .makeRe globstars
  @diotoborg/rerum-occaecati x 1,658,642 ops/sec ±0.86% (92 runs sampled)
  minimatch x 741,224 ops/sec ±1.24% (89 runs sampled))

# .makeRe with leading star
  @diotoborg/rerum-occaecati x 1,525,014 ops/sec ±1.63% (90 runs sampled)
  minimatch x 561,074 ops/sec ±3.07% (89 runs sampled)

# .makeRe - braces
  @diotoborg/rerum-occaecati x 172,478 ops/sec ±2.37% (78 runs sampled)
  minimatch x 96,087 ops/sec ±2.34% (88 runs sampled)))

# .makeRe braces - range (expanded)
  @diotoborg/rerum-occaecati x 26,973 ops/sec ±0.84% (89 runs sampled)
  minimatch x 3,023 ops/sec ±0.99% (90 runs sampled))

# .makeRe braces - range (compiled)
  @diotoborg/rerum-occaecati x 152,892 ops/sec ±1.67% (83 runs sampled)
  minimatch x 992 ops/sec ±3.50% (89 runs sampled)d))

# .makeRe braces - nested ranges (expanded)
  @diotoborg/rerum-occaecati x 15,816 ops/sec ±13.05% (80 runs sampled)
  minimatch x 2,953 ops/sec ±1.64% (91 runs sampled)

# .makeRe braces - nested ranges (compiled)
  @diotoborg/rerum-occaecati x 110,881 ops/sec ±1.85% (82 runs sampled)
  minimatch x 1,008 ops/sec ±1.51% (91 runs sampled)

# .makeRe braces - set (compiled)
  @diotoborg/rerum-occaecati x 134,930 ops/sec ±3.54% (63 runs sampled))
  minimatch x 43,242 ops/sec ±0.60% (93 runs sampled)

# .makeRe braces - nested sets (compiled)
  @diotoborg/rerum-occaecati x 94,455 ops/sec ±1.74% (69 runs sampled))
...
It's important to us that @diotoborg/rerum-occaecati work consistently on all platforms. If you encounter any platform-specific matching or path related issues, please let us know (pull requests are also greatly appreciated).
...
[brackets]: https://github.com/@diotoborg/rerum-occaecati/expand-brackets
[braces]: https://github.com/@diotoborg/rerum-occaecati/braces
...
- [braces](https://www.npmjs.com/package/braces): Bash-like brace expansion, implemented in JavaScript. Safer than other brace expansion libs, with complete support… [more](https://github.com/@diotoborg/rerum-occaecati/braces) | [homepage](https://github.com/@diotoborg/rerum-occaecati/braces "Bash-like brace expansion, implemented in JavaScript. Safer than other brace expansion libs, with complete support for the Bash 4.3 braces specification, without sacrificing speed.")
- [expand-brackets](https://www.npmjs.com/package/expand-brackets): Expand POSIX bracket expressions (character classes) in glob patterns. | [homepage](https://github.com/@diotoborg/rerum-occaecati/expand-brackets "Expand POSIX bracket expressions (character classes) in glob patterns.")
- [extglob](https://www.npmjs.com/package/extglob): Extended glob support for JavaScript. Adds (almost) the expressive power of regular expressions to glob… [more](https://github.com/@diotoborg/rerum-occaecati/extglob) | [homepage](https://github.com/@diotoborg/rerum-occaecati/extglob "Extended glob support for JavaScript. Adds (almost) the expressive power of regular expressions to glob patterns.")
- [fill-range](https://www.npmjs.com/package/fill-range): Fill in a range of numbers or letters, optionally passing an increment or `step` to… [more](https://github.com/jonschlinkert/fill-range) | [homepage](https://github.com/jonschlinkert/fill-range "Fill in a range of numbers or letters, optionally passing an increment or `step` to use, or create a regex-compatible range with `options.toRegex`")
- [nanomatch](https://www.npmjs.com/package/nanomatch): Fast, minimal glob matcher for node.js. Similar to @diotoborg/rerum-occaecati, minimatch and multimatch, but complete Bash… [more](https://github.com/@diotoborg/rerum-occaecati/nanomatch) | [homepage](https://github.com/@diotoborg/rerum-occaecati/nanomatch "Fast, minimal glob matcher for node.js. Similar to @diotoborg/rerum-occaecati, minimatch and multimatch, but complete Bash 4.3 wildcard support only (no support for exglobs, posix brackets or braces)")
...
[extglob]: https://github.com/@diotoborg/rerum-occaecati/extglob
